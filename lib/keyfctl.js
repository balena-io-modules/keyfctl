...
module.exports = class Keyfctl {
  static lint(base, head, cwd) {
    const out = {
      valid: true,
      messages: [],
      frames: []
    ...
    return Promise.each([ new Keyframe(base, cwd), new Keyframe(head, cwd) ], keyframe => {
      return keyframe.loadData
    })
    .then(([base, head]) => {
      if (base.raw === head.raw) {
        out.messages.push('No change in keyframe')
        return out
      }

      if (!head.validate()) {
        out.messages.push(head.errors)
        out.valid = false
        return out
      }

      _.forEach(head.services(), service => {
        const baseService = base.services()[service.name()]

        if (!baseService) {
          service.action = `create service ${service.name()} at ${service.version}`
          out.frames.push(service)
          return
        }

        const differences = service.differencesFrom(baseService)
        ...
        // otherwise, must be an update action
        service.action = `update service ${service.name()} from ${baseService.version} to ${service.version} because: ` + differences.join('; ')

        out.frames.push(service)
      })

      _.forEach(base.services(), service => {
        if (head.services()[service.name()]) {
          return
        }

        service.action = `delete service ${service.name()} at ${service.version}`
        out.frames.push(service)
      })

      out.messages = _.map(out.frames, frame => frame.action)
      return out
    })
  }

  // get the set of commits relevant to the keyframe file
  generateFrames(revision, end) {
